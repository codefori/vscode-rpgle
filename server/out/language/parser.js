...
const declaration_1 = require("./models/declaration");
...
class Parser {
    ...
    async fetchTable(name, keyVersion = ``, aliases) {
        ...
        if (this.tables[existingVersion]) {
            ...
            if (this.tables[existingVersion].fetching)
                return [];
    ...
    async getDocs(workingUri, content, options = { withIncludes: true }) {
        ...
        let dsScopes = [];
        // Global scope bits
        scopes.push(new cache_1.default());
        /**
        ...
        for (const file of Object.keys(files)) {
            ...
            for (let line of files[file]) {
                ...
                if (isFullyFree || lineIsFree) {
                    ...
                    switch (parts[0]) {
                        case `DCL-F`:
                            if (currentItem === undefined) {
                                currentItem = new declaration_1.default(`file`);
                                ...
                                if (recordFormats.length > 0) {
                                    ...
                                    recordFormats.forEach(recordFormat => {
                                        ...
                                        if (qualified)
                                            recordFormat.keywords.push(`QUALIFIED`);
                        ...
                        case `DCL-C`:
                            if (currentItem === undefined) {
                                currentItem = new declaration_1.default(`constant`);
                                currentItem.name = partsLower[1];
                                currentItem.keywords = parts.slice(2);
                                currentItem.description = currentDescription.join(` `);
                                currentItem.position = {
                        ...
                        case `DCL-S`:
                            if (currentItem === undefined) {
                                currentItem = new declaration_1.default(`variable`);
                                currentItem.name = partsLower[1];
                                currentItem.keywords = parts.slice(2);
                                currentItem.description = currentDescription.join(` `);
                        ...
                        case `DCL-DS`:
                            if (currentItem === undefined) {
                                currentItem = new declaration_1.default(`struct`);
                                currentItem.name = partsLower[1];
                                currentItem.keywords = parts.slice(2);
                                currentItem.description = currentDescription.join(` `);
                                currentItem.tags = currentTags;
                                ...
                                if (currentItem.keywords.some(keyword => oneLineTriggers_1.default[`DCL-DS`].some(trigger => keyword.startsWith(trigger)))) {
                        ...
                        case `DCL-PR`:
                            if (currentItem === undefined) {
                                if (!scope.procedures.find(proc => proc.name.toUpperCase() === parts[1])) {
                                    ...
                                    currentItem = new declaration_1.default(`procedure`);
                                    currentItem.name = partsLower[1];
                                    currentItem.keywords = parts.slice(2);
                                    currentItem.description = currentDescription.join(` `);
                                    currentItem.tags = currentTags;
                                    ...
                                    // Does the keywords include a keyword that makes end-ds useless?
                                    if (currentItem.keywords.some(keyword => oneLineTriggers_1.default[`DCL-PR`].some(trigger => keyword.startsWith(trigger)))) {
                                        currentItem.range.end = lineNumber;
                        ...
                        case `DCL-PROC`:
                            ...
                            if (existingProc >= 0)
                                scope.procedures.splice(existingProc, 1);
                            currentItem = new declaration_1.default(`procedure`);
                        ...
                        case `BEGSR`:
                            if (!scope.subroutines.find(sub => sub.name.toUpperCase() === parts[1])) {
                                currentItem = new declaration_1.default(`subroutine`);
                                currentItem.name = partsLower[1];
                                currentItem.description = currentDescription.join(` `);
                                currentItem.position = {
                                    path: file,
                                ...
                                if (currentItem && [`procedure`, `struct`].includes(currentItem.type)) {
                                    if (currentItem.readParms && parts.length > 0) {
                                        if (parts[0].startsWith(`DCL`))
                                            parts.slice(1);
                                        currentSub = new declaration_1.default(`subitem`);
                    ...
                    switch (spec) {
                        case `F`:
                            const fSpec = (0, fixed_1.parseFLine)(line);
                            ...
                            break;
                        case `C`:
                            const cSpec = (0, fixed_1.parseCLine)(line);
                            ...
                            break;
                        case `P`:
                            const pSpec = (0, fixed_1.parsePLine)(line);
                            ...
                            break;
                        case `D`:
                            const dSpec = (0, fixed_1.parseDLine)(line);
                            ...
                            if (dSpec.potentialName.endsWith(`...`)) {
                                ...
                                switch (dSpec.field) {
                                    ...
                                    case `S`:
                                        currentItem = new declaration_1.default(`variable`);
                                        currentItem.name = potentialName || `*N`;
                                        currentItem.keywords = [(0, fixed_1.getPrettyType)(dSpec), ...dSpec.keywords];
                                        // TODO: line number might be different with ...?
                                        currentItem.position = {
                                            path: file,
                                            line: lineNumber - (potentialNameUsed ? 1 : 0) // Account that name is on line before
                                    ...
                                    case `PI`:
                                        ...
                                        if (currentProcName) {
                                            ...
                                            currentGroup = `procedures`;
                                            if (currentItem) {
                                                currentItem.keywords.push((0, fixed_1.getPrettyType)(dSpec), ...dSpec.keywords);
                                            }
                                        }
                                        break;
                                    default:
                                        ...
                                        if (!currentItem) {
                                            switch (currentGroup) {
                                                ...
                                                case `procedures`:
                                                    ...
                                                    for (let i = scopes.length - 1; i >= 0; i--) {
                                                        validScope = scopes[i];
                                                        if (validScope[currentGroup].length > 0)
                                                            break;
                                                    }
                                        ...
                                        if (currentItem) {
                                            ...
                                            if (potentialName) {
                                                ...
                                                currentSub.keywords = [(0, fixed_1.getPrettyType)(dSpec), ...dSpec.keywords];
                                                currentSub.position = {
                                                    path: file,
                                                    line: lineNumber
                                                };
                                                ...
                                                if (currentItem) {
                                                    if (currentItem.subItems.length > 0)
                                                        currentItem.subItems[currentItem.subItems.length - 1].keywords.push((0, fixed_1.getPrettyType)(dSpec), ...dSpec.keywords);
                                                    else
    ...
    static expandKeywords(parts) {
        const keyvalues = {};
        if (parts.length > 0) {
            const keywordParts = (0, statement_1.createBlocks)((0, statement_1.parseStatement)(parts.join(` `)));
