...
exports.createBlocks = exports.fixStatement = exports.parseStatement = void 0;
...
const commonMatchers = [
    ...
    },
    {
        ...
        match: [
            { type: `asterisk` },
            { type: `word`, match: (word) => [`CTDATA`, `BLANK`, `BLANKS`, `ZERO`, `ZEROS`, `ON`, `OFF`, `NULL`, `ISO`, `MDY`, `DMY`, `EUR`, `YMD`, `USA`, `SECONDS`, `S`, `MINUTES`, `MN`, `HOURS`, `H`, `DAYS`, `D`, `MONTHS`, `M`, `YEARS`, `Y`, `HIVAL`, `END`, `LOVAL`, `START`, `N`, `OMIT`, `STRING`, `CWIDEN`, `CONVERT`].includes(word.toUpperCase()) || word.toUpperCase().startsWith(`IN`)
            }
        ],
        becomes: {
            type: `special`
 ...
 */
function fixStatement(statement) {
    for (let i = 0; i < statement.length; i++) {
...
exports.fixStatement = fixStatement;
function createBlocks(statement) {
    let start = 0;
    let level = 0;
    for (let i = 0; i < statement.length; i++) {
