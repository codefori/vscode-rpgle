...
function findAllRelatedKeywords(
  ...
  // Build regex for all block keywords
  const allKeywords = [...pair.open, ...pair.close, ...(pair.middle || [])];
  const regex = new RegExp(`\\b(${allKeywords.join('|')})\\b`, 'gi');
  
  // Find all matches in document, excluding comments
  const allMatches: { offset: number; word: string; length: number }[] = [];
  let match;
  regex.lastIndex = 0;
...
function findMatchingClose(
  ...
  for (let i = openIndex; i < matches.length; i++) {
    const word = matches[i].word;
    
    if (pair.open.includes(word)) {
      depth++;
    } else if (pair.close.includes(word)) {
      depth--;
      if (depth === 0) {
        return i;
      }
...
function findMatchingOpen(
  ...
  // Start with depth 1 for both middle and closing keywords
