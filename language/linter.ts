      ...
      if (currentIndent >= 0) {
        ...
        if (doc.statements[si-1] === undefined || (doc.statements[si-1].range.line !== lineNumber)) {
          if (indentEnabled && skipIndentCheck === false) {
            ...
            if ([
              `ENDIF`, `ENDFOR`, `ENDDO`, `ELSE`, `ELSEIF`, `ON-ERROR`, `ENDMON`, `ENDSR`, `WHEN`, `WHEN-IS`, `WHEN-IN`, `OTHER`, `END-PROC`, `END-PI`, `END-PR`, `END-DS`, `ENDSL`
            ].includes(opcode)) {
            ...
            if ([
              `IF`, `ELSE`, `ELSEIF`, `FOR`, `FOR-EACH`, `DOW`, `DOU`, `MONITOR`, `ON-ERROR`, `ON-EXIT`, `BEGSR`, `SELECT`, `WHEN`, `WHEN-IS`, `WHEN-IN`, `OTHER`, `DCL-PROC`, `DCL-PI`, `DCL-PR`, `DCL-DS`
            ].includes(opcode)) {
