...
if (a > 0);           // Stack: [if]
  ...
  // Note: the dcl-ds likeds()/likerec() bracket-matching behaviour is covered by
  // the `isSingleLineDclDs` unit tests in foldingRange.test.ts, which exercise the
  // exact helper the bracket matcher calls to decide whether a dcl-ds opens a block.
});
